...
from memory_vault.models import MemoryChunk, MemoryConfig, MemoryType, PaginatedResult, RecallResult
...
def test_memory_vault_requires_user_id() -> None:
    with pytest.raises(ValueError):
        MemoryLayer(user_id="")


def test_memory_vault_supports_qdrant_backend() -> None:
    sdk = MemoryLayer(
        user_id="user_a",
        config=MemoryConfig(
...
def test_memory_vault_rejects_unsupported_metadata_backend() -> None:
...
async def test_compress_delegates_to_compressor(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    ...
    monkeypatch.setattr("memory_vault.sdk.MemoryCompressor", FakeCompressor)
